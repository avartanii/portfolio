...
def testCount():
    ...
    printQuery(Count(in_headers,['minutes > 15000']), lns)
...
class Rename:
    ...
    def __init__(self, in_headers, args):
        ...
        self.aggregate_headers = []
        if args[0] in self.input_headers:
            self.output_headers[self.input_headers.index(args[0])] = args[1]

    def process_row(self,row):
        rowString = rowDictToString(self.input_headers, row)
        return rowStringToDict(self.output_headers, rowString)
...
class Swap:
    ...
    def __init__(self, in_headers, args):
        ...
        self.output_headers = list(in_headers)
        self.aggregate_headers = []
        if args[0] in self.input_headers and args[1] in self.input_headers:
            self.output_headers[self.input_headers.index(args[0])] = args[1]
            self.output_headers[self.input_headers.index(args[1])] = args[0]
...
class Select:
    ...
    def __init__(self, in_headers, args):
        ...
        self.aggregate_headers = []
        for a in args:
            if a in self.input_headers:
                self.output_headers.append(a)
...
class Update:
    ...
    def __init__(self, in_headers, args):
        ...
        if self.conditions[0] not in self.input_headers:
            raise Exception("COLUMN DNE")

    def process_row(self,row):
        evalled = eval(self.conditions[1], row)
        row[self.conditions[0]] = evalled
        return row
...
class Add:
    ...
    def __init__(self, in_headers, args):
        ...
        self.aggregate_headers = []
        self.conditions = args
        if self.conditions[0] in self.input_headers:
            raise Exception("COLUMN ALREADY EXISTS")
        else:
            self.output_headers.append(self.conditions[0])
...
def testAdd():
    ...
    """
    (in_headers,lns) = openCSVFile('player_career_short.csv')
    query = Add(in_headers, ['ppg', 'int(int(pts)/int(gp))'])
    printQuery(query,lns)

    # (in_headers,lns) = openCSVFile('player_career_short.csv')
    # q2 = Add(in_headers, ['fullName', "firstname + ' ' + lastname"])
    # printQuery(q2,lns)

    # (in_headers,lns) = openCSVFile('people2.csv')
    # query = Add(in_headers, ['double_age', 'int(Age) * 2'])
    # printQuery(query,lns)
...
class MaxBy:
    ...
    def __init__(self, in_headers, args):
        ...
        self.max = 0.0
        self.maxId = None
        if self.conditions[0] not in self.input_headers or self.conditions[1] not in self.input_headers:
            raise Exception("COLUMN DNE")
        else:
            self.aggregate_headers = ['Max ' + self.conditions[0] + ' By ' + self.conditions[1]]

    def process_row(self,row):
        if (float(row[self.conditions[1]]) > float(self.max)):
            self.max = float(row[self.conditions[1]])
            self.maxId = row[self.conditions[0]]
        return row
    
    def get_aggregate(self):
        return {'Max ' + self.conditions[0] + ' By ' + self.conditions[1] : self.maxId}
...
class Sum:
    ...
    def __init__(self, in_headers, args):
        ...
        if self.conditions[0] not in self.input_headers:
            ...
            self.aggregate_headers = [self.conditions[0] + ' Sum']

    def process_row(self,row):
        self.sum += int(row[self.conditions[0]])
        return row

    def get_aggregate(self):
        return {self.conditions[0] + ' Sum' : self.sum}
...
class Mean:
    ...
    def __init__(self, in_headers, args):
        ...
        if self.conditions[0] not in self.input_headers:
            ...
            self.aggregate_headers = [self.conditions[0] + ' Mean']

    def process_row(self,row):
        self.count += 1
        self.total += int(row[self.conditions[0]])
        #self.mean = self.mean + ((int(row[self.conditions[0]]) - self.mean) / self.count)
        


        # self.count += 1
        # self.mean *= self.count - 1
        # self.mean += int(row[self.conditions[0]])
        # self.mean /= self.count
        # self.mean = round(self.mean)
        return row

    def get_aggregate(self):
        self.mean = round(self.total / self.count)
        return {self.conditions[0] + ' Mean' : self.mean}
...
class ComposeQueries:
    ...
    def __init__(self, q1, q2):
        if (q1.output_headers != q2.input_headers):
            ...
            self.aggregate_headers = self.qu1.aggregate_headers + self.qu2.aggregate_headers

    def process_row(self,row):
        processedQ1 = self.qu1.process_row(row)
        if processedQ1 is not None:
            return self.qu2.process_row(processedQ1)
        else:
            return processedQ1

    def get_aggregate(self):
        ag1 = dict(self.qu1.get_aggregate())
        ag2 = dict(self.qu2.get_aggregate())
        ag1.update(ag2)
        return ag1
...
def buildQuery(in_headers, args):
    ...
    """
    argsList = splitArgs(args)
    queryList = []
    next_in_headers = in_headers
    finalQuery = None

    for a in argsList:
        queryName = queries[a[0][1:]]
        newQuery = queryName(next_in_headers, a[1 : 3])
        next_in_headers = newQuery.output_headers
        queryList.append(newQuery)

    finalQuery = ComposeQueries(queryList[0], queryList[1])
    queryList = queryList[2:]

    for q in queryList:
        finalQuery = ComposeQueries(finalQuery, q)

    return finalQuery


    # for i in range(0,len(qStrings)):
    #     q1 = queries[qStrings[i]](headers, hStrings[i])  
    #     headers = q1.output_headers
...
def testBuildQueries():
    ...
    # get the input headers
    in_headers = f.readline().strip().split(',')
    query = buildQuery(in_headers, ['-Add', 'stealsPerGame', 'int(stl)/int(gp)', 
        '-Filter', 'stealsPerGame > .5', '-Count'])
...
def mostPointsPerMinute():
    """
    Write a query that returns the full name (firstname + ' ' + lastname)
    of the player who has the most points per minute. 
    """
    f = open('player_career.csv')

    # get the input headers
    in_headers = f.readline().strip().split(',')

    query = buildQuery(in_headers, ['-Add', 'pointsPerMinute', 'int(pts)/int(minutes)', 
        '-Add', 'fullName', "firstname + ' ' + lastname", 
        '-MaxBy', 'fullName', 'pointsPerMinute'])
...
def mosts():
    ...
    query = buildQuery(in_headers, ['-Add', 'fullName', "firstname + ' ' + lastname", 
        '-MaxBy', 'fullName', 'gp', 
        '-MaxBy', 'fullName', 'minutes', 
        '-MaxBy', 'fullName', 'pts', 
        '-MaxBy', 'fullName', 'oreb', 
        '-MaxBy', 'fullName', 'dreb', 
        '-MaxBy', 'fullName', 'reb', 
...
def bestPercentages():
    ...
    query = buildQuery(in_headers, ['-Add', 'fieldGoalPercentage', 'int(fgm)/int(fga)', 
        '-Add', 'freeThrowPercentage', 'int(ftm)/int(fta)', 
        '-Add', 'threePointPercentage', 'int(tpm)/int(tpa)', 
        '-Add', 'fullName', "firstname + ' ' + lastname", 
...
def count50Percents():
    ...
    query = buildQuery(in_headers, ['-Add', 'over50', '(float(fgm)/float(fga) > .5)', 
        '-Filter', 'over50 is True', '-Count'])
